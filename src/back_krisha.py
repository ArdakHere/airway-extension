...
# it's better to make it into a table or something so that we don't have to read it every time a user makes a request  # noqa: E501
...
# too wordy name, and_return_object_count
# should just take a dict inside
def make_2gis_request_and_return_object_count(
    ...
    if object_type != "":
        ...
        base_url = f"https://catalog.api.2gis.com/3.0/items?q={object_to_search}&point={location}&radius={radius}&type={object_type}&key={apiKey}"  # noqa: E501
        ...
        base_url = f"https://catalog.api.2gis.com/3.0/items?q={object_to_search}&point={location}&radius={radius}&key={apiKey}"  # noqa: E501
...
def calculate_air_quality_index(metrics_data: dict) -> dict:
    ...
    # can be cast into a function from a utils file, smth like get_color()
    if aq_index >= 100:
        color = [255, 119, 0]
    if aq_index >= 85:
        color = [255, 189, 55]
...
def access_metrics(coords: dict) -> dict:
    ...
    # should be optimized: u can make this dict right away
    data_processed = {}
